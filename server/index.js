...
function sendMail(user, ele, list){
    function get_info([user, ele, list], callback){
        var sql = "SELECT utilizadores.email, eleicoes.nome, listas.descricao FROM utilizadores, eleicoes, listas, votos WHERE utilizadores.id_user=? AND eleicoes.id_ele = ? AND listas.id_listas=? GROUP BY utilizadores.email"
    ...
    get_info([user, ele, list], function(result){
        ...
        let message = {
            ...
            text: "O seu voto no evento " + result.nome + " na lista "+ result.descricao + " foi registado com sucesso.",
        };
...
app.get( "/voted/:userid/:eleid", (req, res) => {
    ...
    db.query("SELECT * FROM votos WHERE utilizador_id = ? AND eleicao_id = ?", [userid, eleid], (err, result) => {
        if (err) {
            res.send({err: err});
        } else {
            res.send(result);
...
app.get("/events/:id", (req, res) => {
    ...
    const sqlEvent = "SELECT eleicoes.*,grupos.d_nome FROM eleicoes, grupos WHERE eleicoes.distrito=grupos.d_id AND grupos.d_id=?";
...
app.get("/generalEvents", (req, res) => {
    const sqlEvents = "SELECT eleicoes.*,grupos.d_nome FROM eleicoes, grupos WHERE eleicoes.distrito=grupos.d_id AND grupos.d_id=?";
...
app.get("/getvotes/:id", (req, res) => {
    ...
    const sqlVotesTotal = "SELECT listas.descricao AS listanome, COUNT(*) AS countvotos FROM votos LEFT JOIN listas ON listas.id_listas = votos.lista_id WHERE votos.eleicao_id=? GROUP BY lista_id;"
...
app.get("/getnovotes/:id", (req, res) => {
    ...
    const sqlNoVotes = "SELECT COUNT(*) as total FROM utilizadores JOIN eleicoes ON eleicoes.distrito = utilizadores.distrito LEFT JOIN votos ON votos.utilizador_id = utilizadores.id_user AND votos.eleicao_id = eleicoes.id_ele WHERE eleicoes.id_ele = ? AND votos.utilizador_id IS NULL";
...
app.get("/getpublic/:id", (req, res) => {
    const ele = req.params.id;
    const sqlPublicVoters = "SELECT votos.*, utilizadores.nome, listas.descricao FROM votos, utilizadores, listas WHERE votos.anonim = 0 AND votos.utilizador_id=utilizadores.id_user AND votos.lista_id=listas.id_listas AND votos.eleicao_id=?";
...
app.post("/update",  (req, res) => {
    ...
    db.query(sqlUpdate, [nome, email, password, doc_nr, doc_type, userid] , (err, result)=> {
        if (err) {
            res.send({err: err});
        } else {
            res.send(result);
