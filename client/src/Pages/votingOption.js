...
function VotingOption() {
  ...
  const submitvote = (idlista) => {
    ...
    Axios.post(`https://atwvc.herokuapp.com/vote`, {
      userid: userid,
      idele: id.id,
      listid: idlista,
      anonim: anonimo,
      ...
      if (response.data.affectedRows === 1) {
        setMsg(
          <Alert variant="success">
            <Alert.Heading>Voto submetido com sucesso!</Alert.Heading>
          </Alert>
        );
        history.goBack();
      } else {
        setMsg(
          <Alert variant="danger">
            <Alert.Heading>Falha na votação!</Alert.Heading>
          </Alert>
        );
      }
    });
  ...
  useEffect(() => {
    Axios.get(`https://atwvc.herokuapp.com/getlists/${id.id}`).then(
      (response) => {
