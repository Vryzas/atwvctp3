...
function Profile() {
  ...
  if (docData.length === 0) {
    if (docData !== null) {
      Axios.post('https://atwvc.herokuapp.com/getDocs').then((response) => {
  ...
  useEffect(() => {
    Axios.get(`https://atwvc.herokuapp.com/isLoggedIn/${id.id}`).then(
      (response) => {
        setUserData({
  ...
  const updateUserData = () => {
    Axios.post('https://atwvc.herokuapp.com/update', {
      user_id: id.id,
      nameuser: nameuser,
      mailAd: mailAd,
      passw: passw,
      ...
      if (response.data.affectedRows === 1) {
        setMsg(
          <Alert variant="success">
            <Alert.Heading>Dados alterados com sucesso!</Alert.Heading>
          </Alert>
        );
      } else {
        setMsg(
          <Alert variant="danger">
            <Alert.Heading>Falha na alteração!</Alert.Heading>
          </Alert>
        );
      }
    });
